...
def default_parse_args(stage='train'):
    ...
    if stage == 'train':
        ...
        parser.add_argument('--snapshot', default=None, help='load from snapshot')
        parser.add_argument('--test_iter', type=int, default=0, help='test iteration')
    elif stage == 'eval':
        ...
        parser.add_argument('--random_ransac_iters', type=int, default=None, help='number for Ransac iters')
    else:
        RuntimeError
    ...
    if stage == 'test' or stage == 'eval':
        if osp.exists(osp.join(osp.dirname(osp.dirname(osp.dirname(osp.realpath(__file__)))), 'output', '3DMatch_'+args.note_name, 'config.yaml')):
            config_path = osp.join(osp.dirname(osp.dirname(osp.dirname(osp.realpath(__file__)))), 'output', '3DMatch_'+args.note_name, 'config.yaml')
    ...
    # data path
    config['data'] = {}
    config['data']['dataset_root'] = osp.join(config['root_dir'], 'data', '3DMatch')
    # math compute
