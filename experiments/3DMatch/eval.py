...
from geotransformer.modules.registration import weighted_procrustes, registration_with_mmc
...
def eval_one_epoch(args, cfg, logger):
    ...
    scene_roots = list(filter(lambda x: not osp.isfile(x), scene_roots))
        ...
        for file_name in file_names:
            ...
            if args.method == 'lgr':
                ...
                estimated_transform = registration_with_ransac_from_correspondences(
                    ...
                    num_validations=cfg.ransac.num_validations,
                )
                ...
                src_corr_points = torch.from_numpy(src_corr_points).cuda()
                corr_scores = torch.from_numpy(corr_scores).cuda()
                estimated_transform = registration_with_mmc(ref_corr_points, src_corr_points, corr_scores)
                estimated_transform = estimated_transform.detach().cpu().numpy()
            else:
...
def main():
    args, cfg = default_parse_args('eval')
