...
from geotransformer.extensions.chamfer_dist import ChamferDistanceL1, ChamferDistanceL2
...
class FineMatchingLoss(nn.Module):
    ...
    def forward(self, output_dict, data_dict):
        ...
        labels[:, :-1, :-1] = gt_corr_map      # Z(x, y)
        labels[:, :-1, -1] = slack_row_labels  # Z(x, mi+1)
        labels[:, -1, :-1] = slack_col_labels  # Z(ni+1, y)
...
class OverallLoss(nn.Module):
    def __init__(self, cfg):
        ...
        self.weight_rec_loss = cfg.loss.weight_rec_loss
        if self.weight_rec_loss > 0:
    ...
    def forward(self, output_dict, data_dict):
        ...
        if self.weight_rec_loss > 0:
            rec_loss = self.chamfer_loss(output_dict['src_mask_kp'], output_dict['src_mask_kp_warped']) + \
                       self.chamfer_loss(output_dict['tgt_mask_kp'], output_dict['tgt_mask_kp_warped'])
            loss = loss + self.weight_rec_loss * rec_loss
        else:
            rec_loss = 0.0
