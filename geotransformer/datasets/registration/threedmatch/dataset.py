...
from geotransformer.datasets.registration.threedmatch.utils import get_num_fragments, get_est_logs
...
class ThreeDMatchPairDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.est_root_path = est_root_path
    ...
    def __getitem__(self, index):
        ...
        if self.est_root_path is not None:
            est_indices, est_logs = get_est_logs(os.path.join(self.est_root_path, self.subset, metadata['scene_name']), get_num_fragments(metadata['scene_name']))
            last_transform = est_logs[est_indices[metadata['frag_id0'], metadata['frag_id1']]]['transform']
            cur_transform = np.matmul(get_transform_from_rotation_translation(rotation, translation), np.linalg.inv(last_transform))
            rotation, translation = get_rotation_translation_from_transform(cur_transform)
            src_points = apply_transform(src_points, last_transform)
            data_dict['last_transform'] = last_transform.astype(np.float32)
