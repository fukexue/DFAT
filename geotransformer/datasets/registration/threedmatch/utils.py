...
def get_est_logs(gt_root, num_fragments):
    est_logs = read_log_file(osp.join(gt_root, 'est.log'))

    est_indices = -np.ones((num_fragments, num_fragments), dtype=np.int32)
    for i, gt_log in enumerate(est_logs):
        frag_id0, frag_id1 = gt_log['test_pair']
        if frag_id1 > frag_id0 + 1:
            est_indices[frag_id0, frag_id1] = i

    return est_indices, est_logs
