...
class GeometricTransformerDecoder(nn.Module):
    def __init__(
        ...
        """
        super(GeometricTransformerDecoder, self).__init__()
    ...
    def forward(
        ...
        """
        ref_len = ref_points.shape[1]
        src_len = src_points.shape[1]
        ref_points = torch.cat([ref_points, ref_mask_points], dim=1)
        src_points = torch.cat([src_points, src_mask_points], dim=1)
        ref_embeddings = self.embedding(ref_points)
        src_embeddings = self.embedding(src_points)

        ref_feats = torch.cat([ref_feats, ref_mask_tokens], dim=1)
        src_feats = torch.cat([src_feats, src_mask_tokens], dim=1)
        ...
        ref_feats = ref_feats[:, ref_len:, :]
        src_feats = src_feats[:, src_len:, :]
