...
class BaseTrainer(abc.ABC):
    def __init__(
        ...
        if isinstance(cfg.seed, str) and cfg.seed.lower() == 'none':
            self.seed = int(random.random()*100000)
            self.logger.info(f'Seed in cfg is None,  we set (random seed: {self.seed})')
            # update seed in output/config file
            with open(osp.join(cfg.output_dir, 'config.yaml'), 'r') as f:
                content = f.read()
            new_content = content.replace('seed: None', f'seed: {self.seed}')
            with open(osp.join(cfg.output_dir, 'config.yaml'), 'w') as f:
                f.write(new_content)
        else:
    ...
    def load_weight(self, snapshot, fix_prefix=True):
        self.logger.info('Loading from "{}".'.format(snapshot))
        state_dict = torch.load(snapshot, map_location=torch.device('cpu'))
